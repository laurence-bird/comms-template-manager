...
class AssetProcessingSpec extends FlatSpec with Matchers {
  ...
  it should "process uploaded email files" in {
    ...
    val bodyText = UploadedTemplateFile("email/body.txt", "fsfdsfs".getBytes, Email, TextBody)
    val sender   = UploadedTemplateFile("email/sender.txt", "Test <testing@test.com>".getBytes, Email, Sender)
    val asset1   = UploadedTemplateFile("email/assets/image.png", "fsfdsfs".getBytes, Email, Asset)
    val asset2   = UploadedTemplateFile("email/assets/something/image.png", "fsfdsfs".getBytes, Email, Asset)
  ...
  it should "process uploaded SMS file" in {
    val smsTextBody = UploadedTemplateFile("sms/body.txt", "some message {{someVar}}".getBytes, SMS, TextBody)
  ...
  it should "process multiple channels" in {
    ...
    val emailAsset  = UploadedTemplateFile("email/assets/image.png", "fsfdsfs".getBytes, Email, Asset)
    val smsTextBody = UploadedTemplateFile("sms/body.txt", "some message {{someVar}}".getBytes, SMS, TextBody)
