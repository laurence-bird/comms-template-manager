...
object AssetProcessing {
  ...
  def processAssets(region: Regions, assetsS3Bucket: String, commManifest: CommManifest, uploadedFiles: List[UploadedFile]): ErrorsOr[ProcessedFiles] = {
  ...
  private def processEmailAssets(region: Regions, assetsS3Bucket: String, commManifest: CommManifest, uploadedFiles: List[UploadedFile]): ValidatedNel[String, ProcessedFiles] = {
    import cats.syntax.traverse._
    import cats.instances.list._
    val assetFiles = UploadedFile.extractAssetEmailFiles(uploadedFiles)
    val processedTemplateFiles = UploadedFile.extractNonAssetEmailFiles(uploadedFiles).traverseU(uploadedFile => {
      replaceAssetReferences(region, assetsS3Bucket, Email, commManifest, uploadedFile.contents).map(contents => uploadedFile.copy(contents = contents))
    })
    processedTemplateFiles.map(ProcessedFiles(_, assetFiles))
  }

  private def replaceAssetReferences(region: Regions, assetsS3Bucket: String, channel: Channel, commManifest: CommManifest, contents: Array[Byte]): ValidatedNel[String, Array[Byte]] = {
  ...
  private def determineS3Endpoint(region: Regions, assetsS3Bucket: String, channel: Channel, commManifest: CommManifest): ValidatedNel[String, String] = {
    if (!Region.getRegion(region).isServiceSupported("s3")) {
      ...
      Valid(s"https://$s3ServiceEndpoint/$assetsS3Bucket/${commManifest.commType.toString.toLowerCase}/${commManifest.name}/${commManifest.version}/${channel.toString.toLowerCase}")
    }
  }
}
